...
// fall out of sync with the picture. Extended from 3.4 to also say which
// models are still loading or failed, and which item, if any, is selected.
...
export function describeExhibit({ items, animating, selectedId }) {
  ...
  parts.push(selected
    ? `The ${selected.data.name.toLowerCase()} is currently selected; its details are in the panel below.`
    : 'Nothing is selected. Click an item, or use a Select button, to see its details.');
