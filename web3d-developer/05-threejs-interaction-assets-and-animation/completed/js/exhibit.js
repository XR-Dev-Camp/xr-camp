// exhibit.js: the exhibit's five items. The same clay pot, woven basket
// ring, and jade stone from 3.1 through 3.4, built from primitives exactly
// as before, plus two real glTF models on two new pedestals: a fox figure
// and a milk truck, both official Khronos Group sample assets. ITEMS stays
// the single source of truth, so describe.js can build its sentences from
// the same facts the scene is built from, for every item, old or new.
...
// guessing one fixed scale number for every model would be wrong for some
// of them. Measuring the model's own bounding box and scaling from that
// works for any model, regardless of its authored size.
function fitAndPlaceModel(object3D, x, pedestalTopY) {
  ...
  // units, so its min/max must be recomputed before they can be used to
  // sit the model's lowest point exactly on the pedestal.
...
// colour map, and any emissive map) store colour the way a photograph
// does, in sRGB; data textures (normal maps, and the combined
// metalness/roughness map) store numbers, not colour, and must stay
// linear, or their values would be wrongly brightened. GLTFLoader already
// sets this correctly for every texture it creates; this function only
// confirms it, and fixes it on any texture where it is somehow missing, so
// the exhibit is correct even if a future change swaps in a texture by
// hand.
function ensureColorSpaces(object3D) {
  object3D.traverse((node) => {
    ...
    // normalMap and roughnessMap/metalnessMap are left exactly as
    // GLTFLoader set them: THREE.NoColorSpace. They are correct already.
  });
}

// Loads both model items' glTF files and swaps each placeholder for the
// real thing as it arrives. Uses Promise.allSettled, not Promise.all, so
// one model failing to load (a bad path, a network error, an offline
// learner) does not stop the other one from appearing. onProgress reports
// combined byte progress across both files; onItemReady and onItemError
// tell main.js which specific item changed, so it can update the
// description and the attribution panel for that one item.
...
// truck does both, for its wheels): the `seen` set makes sure each geometry,
// material, and texture is disposed exactly once, even when traverse()
// visits it by way of more than one mesh.
...
// stops any running AnimationMixer (a mixer holds no GPU resources of its
// own, but a stopped one cannot keep updating a mesh that no longer
// exists).
