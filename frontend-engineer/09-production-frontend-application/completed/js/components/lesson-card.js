...
//
// Copied from Course 2.5. What changed for 2.9, so the card speaks every
// language the app does:
//   - Every word comes from t() (i18n.js); none is written in this file.
//   - "Done" in the template is a <span> that #render() fills in.
//   - The card listens for "localechange", and renders itself again.
// Its structure, styles, attributes, and lesson-toggle event are unchanged.

import { t } from '../i18n.js';
import { SESSION_MINUTES } from '../config.js';

// 600 minutes → "About 10 hours · 14 sessions of 45 minutes", in the current
// language. Two numbers, so two plural choices: two strings, joined.
function describeMinutes(minutes) {
  const hours = Math.max(1, Math.round(minutes / 60));
  const sessions = Math.ceil(minutes / SESSION_MINUTES);
  return `${t('card.hours', { count: hours })} · ${t('card.sessions', { count: sessions, minutes: SESSION_MINUTES })}`;
}
...
template.innerHTML = `
  ...
  <article part="card">
    ...
    <button type="button" part="button" aria-pressed="false"><span class="tick" aria-hidden="true">✓ </span><span class="label">Done</span></button>
  </article>
`;
...
export class LessonCard extends HTMLElement {
  ...
  // Arrow function in a field: "this" is always the card, and the same
  // function can be removed again in disconnectedCallback.
  ...
  // A card taken out of the page stops listening, so it can be garbage collected.
  ...
  get lessonTitle() {
    return this.getAttribute('lesson-title') ?? t('card.untitled');
  }
  ...
  #render() {
    ...
    this.#meta.textContent = minutes > 0 ? describeMinutes(minutes) : t('card.noTime');
    this.#badge.textContent = t(`status.${status}`);
    this.#button.querySelector('.label').textContent = t('card.done');
    // The visible word first (WCAG 2.5.3): "Done: Web Components".
    this.#button.setAttribute('aria-label', t('card.doneLabel', { title: this.lessonTitle }));
