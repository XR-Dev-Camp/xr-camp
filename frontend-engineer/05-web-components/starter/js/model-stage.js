...
export class ModelStage extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // TODO 13: Connect the scene to its description.
    // - The scene is one picture: give it role="img", and an aria-label
    //   from this element's label attribute (or '3D view').
    // - Find the author's description: this.querySelector('[slot="description"]').
    //   If it has an id, set the scene's aria-describedby to that id.
    // This works because there is no shadow root: the scene and the
    // paragraph are in the same document, so the id can be referenced.
    ...
    // TODO 14: Make it controllable, and kind.
    // - Pause button: flip this.#paused, then call this.#updatePause().
    // - Turn left: this.#turn(TURN_STEP, 'left');
    //   Turn right: this.#turn(-TURN_STEP, 'right');
    // - reducedMotion.addEventListener('change', ...): if the learner turns
    //   on "reduce motion" while the page is open, pause.
    // Then test with the keyboard only, and with reduced motion on.
  }
