...
const T = {
  en: {
    ...
    time: (sessions, months) => `${sessions} sessions of 45 min · about ${months} months at 4 a week`,
    ...
    lessonTime: (h, s) => `${h} h · ${s} sessions${s > 20 ? ' · longer lesson' : ''}`,
    ready: '✅ Ready',
    coming: '🚧 Coming soon',
    phase: 'Phase',
    ...
    license: (f) => `Code: [LICENSE-CODE](LICENSE-CODE) (Apache-2.0). Lessons, documentation, and images: [LICENSE-CONTENT](LICENSE-CONTENT) (CC BY-NC-SA 4.0). Commercial and institutional use: [${f.commercial}](${f.commercial}). Third-party material: [${f.notices}](${f.notices}) and [${f.attribution}](${f.attribution}).`,
  ...
  es: {
    ...
    start: (first) => [
      ...
      '**Aprende a pasos cortos:** cada lección se divide en sesiones de 45 minutos, y cada sesión termina con algo que tú hiciste. La mayoría de las lecciones toma cuatro semanas o menos, con cuatro sesiones por semana. Cada fase termina con una habilidad real que puedes usar y mostrar: puedes detenerte después de cualquier fase o seguir adelante. Dos sesiones por semana también está bien: avanza a tu ritmo.',
      `**Visita el sitio web:** [xrcamp.dev](${SITE}).`,
    ],
    pathTitle: 'El camino',
    ...
    time: (sessions, months) => `${sessions} sesiones de 45 min · unos ${months} meses con 4 por semana`,
    ...
    lessonTime: (h, s) => `${h} h · ${s} sesiones${s > 20 ? ' · lección más larga' : ''}`,
    ready: '✅ Lista',
    coming: '🚧 Próximamente',
    phase: 'Fase',
    ...
    license: (f) => `Código: [LICENSE-CODE](LICENSE-CODE) (Apache-2.0). Lecciones, documentación e imágenes: [LICENSE-CONTENT](LICENSE-CONTENT) (CC BY-NC-SA 4.0). Uso comercial e institucional: [${f.commercial}](${f.commercial}). Material de terceros: [${f.notices}](${f.notices}) y [${f.attribution}](${f.attribution}).`,
  ...
  zh: {
    ...
    start: (first) => [
      ...
      '**小步前进：**每节课都分成若干次 45 分钟的学习，每次学习都会以一个你亲手完成的成果结束。按每周 4 次计算，大多数课程只需四周或更短。每个阶段都会让你掌握一项可以使用、可以展示的真实技能：你可以在任何一个阶段后停下，也可以继续前进。每周 2 次也完全可以：按你自己的节奏来。',
      `**访问网站：**[xrcamp.dev](${SITE})。`,
    ],
    pathTitle: '学习路径',
    pathIntro: '八个阶段，从第一次点击到专业开发者。每个阶段都以一个你亲手发布的项目结束，而且每个阶段本身都有价值。',
    pathHead: '| 阶段 | 你将构建 | 时长 | 已完成课程 |',
    time: (sessions, months) => `${sessions} 次 45 分钟的学习 · 每周 4 次约 ${months} 个月`,
    readyCount: (r, t) => `${t} 课中 ${r} 课`,
    lessonsTitle: '全部课程',
    lessonsIntro: '✅ 已完成表示该课程的英文版已经完成（西班牙语和中文翻译正在进行中）。🚧 表示课程正在编写。',
    lessonHead: '| # | 课程 | 时长 | 状态 |',
    lessonTime: (h, s) => `${h} 小时 · ${s} 次学习${s > 20 ? ' · 较长的课程' : ''}`,
    ready: '✅ 已完成',
    coming: '🚧 即将推出',
    phase: '阶段',
    anatomyTitle: '每节课的结构',
    ...
    licenseTitle: '许可协议',
    license: (f) => `代码：[LICENSE-CODE](LICENSE-CODE)（Apache-2.0）。课程、文档和图片：[LICENSE-CONTENT](LICENSE-CONTENT)（CC BY-NC-SA 4.0）。商业和机构使用：[${f.commercial}](${f.commercial})。第三方素材：[${f.notices}](${f.notices}) 和 [${f.attribution}](${f.attribution})。`,
