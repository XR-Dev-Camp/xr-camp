...
export function createScene(container) {
  ...
  //
  // TODO 14a: setRemoteTarget(userId, position). Get (or create) this
  // avatar's entry with ensureRemoteAvatar(userId). Set entry.from to
  // *where the avatar currently, visually, is* — call
  // currentInterpolated(entry, performance.now()), not entry.to — so a fast
  // update that arrives mid-interpolation starts its next tween from where
  // the eye actually is, with no visible snap. Then set entry.to to a copy
  // of `position`, and entry.startTime to performance.now().
  function setRemoteTarget(userId, position) {
    throw new Error('setRemoteTarget is not implemented yet — see TODO 14a');
  }

  // TODO 14b: currentInterpolated(entry, now). Compute
  // t = Math.min(1, (now - entry.startTime) / INTERPOLATION_MS) — how far
  // through the current tween "now" is, capped at 1 so a late render never
  // overshoots. Return { x, y, z, rotationY }, each field the linear
  // interpolation (THREE.MathUtils.lerp(from, to, t)) between entry.from
  // and entry.to's matching field.
  function currentInterpolated(entry, now) {
    throw new Error('currentInterpolated is not implemented yet — see TODO 14b');
  }
