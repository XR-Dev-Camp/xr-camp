...
//      the server the instant it comes back up. (TODO 12)
//   2. Never send position updates faster than the server will accept them
//      (see server/realtime.js's POSITION_MIN_INTERVAL_MS). (TODO 13)
...
// TODO 12a: backoffDelay(attempt). Return a random delay between 0 and
// min(MAX_DELAY_MS, BASE_DELAY_MS * 2 ** attempt) — "exponential backoff
// with full jitter" (the term AWS's Architecture Blog uses for this exact
// formula). The randomness matters: without it, every learner whose
// connection drops at the same moment (a shared campus Wi-Fi hiccup) would
// reconnect in lockstep and hit the server with a burst of attempts at
// exactly the same instants.
function backoffDelay(attempt) {
  throw new Error('backoffDelay is not implemented yet — see TODO 12a');
}
...
export function connectRoom(room, handlers = {}) {
  ...
  // TODO 12b: scheduleReconnect(). If `stopped` is true, do nothing.
  // Otherwise: compute a delay with backoffDelay(attempt), increment
  // `attempt`, call emit('onStatus', ...) with a message mentioning roughly
  // how long the wait is, and setTimeout(open, delay) — remembering the
  // timer in `reconnectTimer` so stop() (below) can cancel it.
  function scheduleReconnect() {
    throw new Error('scheduleReconnect is not implemented yet — see TODO 12b');
  }
  ...
  return {
    // TODO 13: sendPosition(x, y, z, rotationY), throttled to
    // POSITION_SEND_INTERVAL_MS. Read performance.now(); if less than
    // POSITION_SEND_INTERVAL_MS has passed since lastPositionSentAt, return
    // without sending. Otherwise update lastPositionSentAt and send()
    // { type: 'position', x, y, z, rotationY }. A caller may call this on
    // every animation frame (60 times a second) without flooding the
    // connection — extra calls in between are simply dropped, since a
    // position update, unlike a chat message, is only ever interesting for
    // its most recent value.
    sendPosition(x, y, z, rotationY) {
      throw new Error('sendPosition is not implemented yet — see TODO 13');
    },
    sendChat(text) {
      send({ type: 'chat', text });
    },
