...
// --- Block / mute / report (TODO 16) ---------------------------------------
// See the file comment for the distinction: muting is local-only (this
// function alone decides what appears in your own chat log); blocking asks
// the server to stop relaying that person to you at all (net.sendBlock).

// TODO 16a: toggleMute(userId). Add userId to mutedUserIds if it is not
// already there, or remove it if it is (a "toggle"). Re-render the roster
// table afterwards so the row's "(muted)" label and its button's label
// ("Mute" / "Unmute") update.
function toggleMute(userId) {
  throw new Error('toggleMute is not implemented yet — see TODO 16a');
}

// TODO 16b: toggleBlock(userId). Same toggle idea as 16a, but on
// blockedUserIds, and — unlike muting — this one also has to tell the
// server: call net?.sendBlock(userId, blocking) with the new state, so
// server/realtime.js's handleBlock (TODO 8) actually starts or stops
// filtering this person out for you.
function toggleBlock(userId) {
  throw new Error('toggleBlock is not implemented yet — see TODO 16b');
}

// TODO 16c: openReport(userId, username) and closeReport(). openReport
// remembers `userId` in the module-level `reportTargetUserId`, sets
// #report-target-name's text to `username`, un-hides #report-section, and
// moves focus to #report-reason (so a keyboard or screen-reader user lands
// straight on the first field). closeReport sets reportTargetUserId back
// to null, hides #report-section, and calls $('report-form').reset().
function openReport(userId, username) {
  throw new Error('openReport is not implemented yet — see TODO 16c');
}

function closeReport() {
  throw new Error('closeReport is not implemented yet — see TODO 16c');
}
...
async function init() {
  ...
  // TODO 15: the chat form's submit handler. event.preventDefault() first.
  // Clear #chat-errors. Read and trim #chat-input's value; if it is empty,
  // return without sending anything. If `net` is null (not connected —
  // this can happen if the WebSocket has not opened yet, or the server is
  // not running), show a field error ("Not connected to the room yet.")
  // with showFieldErrors and return. Otherwise call net.sendChat(text) and
  // clear the input. Note that this function does not need to render the
  // message itself: the server echoes every chat message back to its own
  // sender too (see server/realtime.js's handleChat), so it will arrive
  // through onChat() like any other message.
  $('chat-form').addEventListener('submit', (event) => {
    throw new Error('The chat form submit handler is not implemented yet — see TODO 15');
  });
