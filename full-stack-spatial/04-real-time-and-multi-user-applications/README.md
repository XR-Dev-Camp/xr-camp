...
Open `server/server.js`. The `upgrade` event is the one place this whole lesson's authentication and room logic actually connects to a real network socket: check the path, call `authenticateUpgrade`, refuse with a plain HTTP response if it fails, otherwise hand the socket to `wss.handleUpgrade` and call `handleConnection`. This is also the first moment you can genuinely test any of it — open two browser tabs once this is done.
