// sessions.js: carried over unchanged from Course 5.2. A session is a
// short-lived, in-memory fact ("this bearer token is currently signed in as
// this account"); it is not data a learner made, so it does not move into
// SQLite along with scenes and annotations. An in-memory session store — a
// Map from a session id to who it belongs to. Nothing here is written to
// disk, so restarting the server
