...
// fetched and stored directly by bundle-store.js (OPFS or the Cache API),
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    if (!self.navigator.connection?.saveData) {
      ...
      // shell still installs, and cacheFirst() below will still save it
...
// TODO 17: WAKE UP AND TELL EVERY OPEN PAGE.
// Listen for the 'sync' event. If `event.tag !== 'resume-downloads'`,
// return early (a service worker can be asked to handle other sync tags in
// a bigger project; this one only ever registers this one). Otherwise, call
// `event.waitUntil(...)` with an async function that: gets every open page
// with `await self.clients.matchAll({ type: 'window' })`, and calls
// `client.postMessage({ type: 'RESUME_DOWNLOADS' })` on each one.
// js/background-sync.js's watchForResume() is already listening for exactly
// this message. This event only ever fires in a browser that supports the
// Background Sync API — everywhere else, the page's own `online` listener
// does the same job while it is open.
