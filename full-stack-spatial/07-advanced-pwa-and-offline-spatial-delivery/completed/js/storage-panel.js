...
// it. `quota` and `usage` are both in bytes, and `usage` covers everything
// this origin has stored anywhere — OPFS, the Cache API, and localStorage
// together, not only what this project's own bundle downloads added.
...
// can be cleared without warning). The browser decides whether to grant
// it — often based on signals like the page being installed or bookmarked
// — and there is no dialog to point at; a learner can only see the result.
// Support: broad in Chromium and Firefox; Safari does not implement the
// Storage API's persistence request as of the current MDN browser-support
// table, so this project always checks for the method before calling it.
