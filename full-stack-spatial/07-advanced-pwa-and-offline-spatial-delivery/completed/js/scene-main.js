...
// Tries the offline-saved file first; only reaches the network (through the
// configured asset base URL, so a regional mirror is honoured here too) if
// this bundle was never downloaded, or this particular file did not save.
async function resolveModelUrl(path) {
  const savedUrl = await readFileUrl(bundleId, path);
  return savedUrl ?? resolveAssetUrl(path);
}
