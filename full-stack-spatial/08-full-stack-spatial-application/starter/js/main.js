...
// see below). TODO 13: wire the description panel, near the bottom of this
// file.
...
// TODO 13 (part 1 of 2): set $('saved-description').textContent to
// sceneData.description if it is set, or 'No description saved yet.'
// otherwise. Use textContent, the same rule this file applies to every
// other stored text (annotations, chat) -- a saved description came from an
// AI draft a person approved, but it is still stored, user-supplied text,
// and js/main.js never trusts stored text enough to use innerHTML.
...
async function init() {
  ...
  // TODO 13 (part 2 of 2): add two click listeners.
  //
  // $('draft-description-button'): if there is no currentScene, return.
  // Otherwise clear $('description-errors'), call
  // requestDescriptionDraft(currentScene.id), and on failure call
  // showFieldErrors(errorsList, body) as every other form in this file
  // does. On success, set $('description-draft').value to body.description
  // and setStatus a short confirmation naming body.provider.
  //
  // $('save-description-button'): if there is no currentScene, return.
  // Read and trim $('description-draft').value; if it is empty, show a
  // field error asking the learner to write or draft one first. Otherwise
  // call saveDescriptionRequest(currentScene.id, description, csrfToken),
  // show its errors on failure, and on success set currentScene to
  // body.scene, call renderSavedDescription(body.scene), and setStatus a
  // short confirmation.
