...
import {
  adminStats, createAnnotation, createScene, describeScene, formatServerError, getScene,
  listMyScenes, login, logout, me, register, requireAuth, saveDescription, sendJson,
} from './routes.js';
...
// all external files (js/main.js, js/net.js, js/scene.js, js/ai.js,
// A-Frame's own aframe.min.js), so nothing here needs it. connect-src
// 'self' also covers
...
const server = createServer(async (req, res) => {
  try {
    ...
    const descriptionMatch = pathname.match(/^\/api\/scenes\/([^/]+)\/description(\/draft)?$/);
    ...
    // FIXED (TODO 11): this capstone's own two routes, wired the same way as
    // every other authenticated route above -- requireAuth first, then the
    // handler in routes.js decides ownership/visibility (see describeScene
    // and saveDescription's own comments).
    if (descriptionMatch && descriptionMatch[2] && req.method === 'POST') {
      const auth = await requireAuth(req);
      if (!auth) return sendJson(res, 401, { error: 'Sign in required.' });
      return await describeScene(req, res, auth, descriptionMatch[1]);
    }

    if (descriptionMatch && !descriptionMatch[2] && req.method === 'PUT') {
      const auth = await requireAuth(req);
      if (!auth) return sendJson(res, 401, { error: 'Sign in required.' });
      return await saveDescription(req, res, auth, descriptionMatch[1]);
    }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  server.listen(PORT, HOST, () => {
    console.log(`Full-stack spatial capstone listening on http://${HOST}:${PORT}`);
  });
}
