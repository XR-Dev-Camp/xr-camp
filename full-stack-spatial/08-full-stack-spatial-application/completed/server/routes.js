...
// FIXED (TODO 9): builds a draft from this scene's own name and its already
// stored annotations, and returns it -- ai.js never writes it anywhere.
// Nothing here treats the draft as trustworthy fact: it is only ever a
// starting point for the person reviewing it.
export async function describeScene(req, res, auth, sceneId) {
  const scene = findSceneById(sceneId);
  if (!scene || !canView(scene, auth.user.id)) return sendJson(res, 404, { error: 'Scene not found.' });
  const annotations = listAnnotationsForScene(scene.id);
  try {
    const draft = await buildDescriptionDraft(scene, annotations);
    sendJson(res, 200, draft);
  } catch (error) {
    sendJson(res, 502, { error: `Could not build a description draft: ${error.message}` });
  }
}

// FIXED (TODO 10): only the scene's owner may save a description -- unlike
// getScene, a scene being public never lets a visitor write to it. The
// description is sanitised exactly like an annotation before it is stored:
// a human approved this text, but it is still user-controlled text, and
// every stored field in this app goes through sanitizeText for the same
// reason (see sanitize.js).
export async function saveDescription(req, res, auth, sceneId) {
  if (!requireCsrf(req, auth.session, res)) return;
  const scene = findSceneById(sceneId);
  if (!scene || scene.ownerId !== auth.user.id) return sendJson(res, 404, { error: 'Scene not found.' });

  const body = await readBodyOr400(req, res);
  if (body === undefined) return;

  const { valid, errors, value } = validateDescription(body);
  ...
  if (description.length === 0) return sendJson(res, 400, { error: 'Invalid description', details: ['description must be 1-500 characters.'] });
  const updated = updateSceneDescription(scene.id, description);
  sendJson(res, 200, { scene: updated });
}
