...
// from Course 5.3, finished for you) owns that — and no fetch() appears in
// scene.js. Every value the AI tools show (a draft description, a search
// result, "data sent") is exactly what the server sent back; this file
// never invents or rewords any of it.
//
// The scene manager, the annotation form, and the always-present 2D
// description are all finished for you below — TODO 17 and TODO 18, near
// the bottom, are this lesson's actual subject.
...
// --- TODO 17: AI description tool --------------------------------------------
// resetDraftBox() and renderSavedDescription() below are finished for you —
// use them from the functions you write.
...
// generateDescription() (called by the "Generate a draft description"
// button) should:
//   1. If there is no currentSceneId, setStatus('Save this arrangement as a
//      scene first.', 'error') and return.
//   2. POST to `/api/scenes/${currentSceneId}/describe` (no body needed).
//   3. On a non-ok response, setStatus(body.error ?? 'Could not generate a
//      description.', 'error') and return.
//   4. On success, un-hide #ai-draft-box, set #ai-draft-textarea's value to
//      body.description, set #ai-draft-provider's text to something like
//      `Drafted by the "${body.provider}" provider from this scene's own
//      data.`, fill #ai-draft-warnings with one <li> per entry in
//      body.warnings (prefix each with "Check this before saving: "), and
//      set #ai-draft-data-sent's textContent to
//      JSON.stringify(body.dataSent, null, 2) — this is what lets a learner
//      see, in the page itself, exactly what left the browser.
//   5. Wrap the fetch in try/catch, setting an 'offline' status on failure,
//      the same way every other network call in this file does.
async function generateDescription() {
  throw new Error('TODO 17: generateDescription is not implemented yet');
}

// saveDescription() (called by "Save this description") should:
//   1. Return if there is no currentSceneId.
//   2. Clear #ai-draft-errors, then PUT #ai-draft-textarea's current value
//      (as { description }) to `/api/scenes/${currentSceneId}/description`.
//   3. On a non-ok response, show the field errors with showFieldErrors.
//   4. On success, update currentScene to the response body,
//      renderSavedDescription(currentScene), resetDraftBox(), and
//      setStatus('Description saved.').
async function saveDescription() {
  throw new Error('TODO 17: saveDescription is not implemented yet');
}

// --- Loading and saving scenes (finished) ------------------------------------
...
// --- TODO 18: natural-language search -----------------------------------------
// renderSearchResults() below is finished for you.
...
// searchScenes() (called by the "Search" button and by pressing Enter in the
// search box) should:
//   1. Clear #search-errors. Read and trim #search-query's value; if it is
//      empty, showFieldErrors(errorsList, { error: 'Type something to
//      search for.' }) and return.
//   2. POST { query } to /api/search.
//   3. On a non-ok response, show the field errors.
//   4. On success, set #search-explanation's text to body.explanation, and
//      — if body.dropped.length > 0 — append a note that that many invented
//      scene ids were dropped (this is the hallucination guard from ai.js,
//      made visible to a learner). Then call
//      renderSearchResults(body.matches).
//   5. Wrap the fetch in try/catch with the usual 'offline' status.
async function searchScenes() {
  throw new Error('TODO 18: searchScenes is not implemented yet');
}
