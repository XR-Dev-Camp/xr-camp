...
The reference solution is in [`completed/`](completed/). The starter is 4.1's finished, WebXR-capable exhibit with three new files (`controllers.js`, `menu.js`, `ar.js`) and small additions to two others: 11 TODOs in total.
...
`renderer.xr.getHand(index)` is the same idea for a tracked hand, and `XRHandModelFactory.createHandModel(hand, 'mesh')` builds a skinned mesh that follows all 25 joints WebXR's Hand Input module reports (a wrist, and four segments for each of five fingers). Passing `'boxes'` or `'spheres'` instead of `'mesh'` draws simple placeholder shapes at each joint instead, useful for testing without downloading a model (see the Explorer challenge).
...
**`XRInputSource.targetRayMode`** is `'gaze'` (a head- or eye-tracked direction, no controller involved), `'tracked-pointer'` (a physically tracked controller or hand), or `'transient-pointer'` (an input generated by the operating system's own interaction intent - a screen tap in an inline or AR session, a platform accessibility gesture, or a source too sensitive to expose directly, such as raw gaze) - not the same thing as `'gaze'`, even though both can stand in for "no hardware controller".
...
| The camera never moves unless the learner (or, in AR, their own device) moves it | 2.3.3 | Unrequested motion is disorienting, and can cause real physical discomfort. |
