...
if (!supportsWebGL2()) {
  ...
  // that nothing plays sound before the learner asks for it.
  // When soundToggle is clicked:
  // - if `isAnyPlaying(pedestalAudio)` is true: call `pauseAll(pedestalAudio)`,
  //   `guideAudio.pause()`, set `aria-pressed` to 'false', and the button's
  //   text back to 'Start pedestal sounds'.
  // - otherwise: call `app.listener.context.resume()` FIRST, then
  //   `playAll(pedestalAudio)` and `guideAudio.play()`, set `aria-pressed`
  //   to 'true', and the button's text to 'Pause pedestal sounds'.
  // Call `updateDescription()` at the end either way.
  ...
  // 'exponential' without restarting anything. Call
  // `setDistanceModel(pedestalAudio, distanceModelSelect.value)`, then
  // `updateDescription()`, whenever the select's value changes.
